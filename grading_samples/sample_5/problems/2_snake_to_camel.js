...
function snakeToCamel(str) {
  ...
  for (var i = 0; i < seperated.length; i++) {
    ...
    for (var j = 0; j < word.length; j++) {
      var letter = word[j] //nice use of intermediate variables - MM
        if (j === 0){
        answer.push(letter.toUpperCase())  // watch indentation when getting this deep - maybe try helper function - MM
        }   else {
